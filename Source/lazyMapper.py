...
# Date: 4/9/2023
# Description: Long Stream Maps Generator v1.0
import os
os.system("title " + os.path.basename(__file__)[0:-3])
ver="v1.0.0"
folderCustom = ['WELUVLAMAS0.5.osu', 'WELUVLAMAS0.6.osu', 'WELUVLAMAS0.7.osu', 'WELUVLAMAS0.8.osu', 'WELUVLAMAS0.9.osu', 'WELUVLAMAS1.0.osu']
...
def printMenu():
    print("-----------------------" *2)
    print("      " +  os.path.basename(__file__)[0:-3])
    print("        " + ver)
    print("   By GokaGokai")
    print("-----------------------" *2)
...
def printMapType():
    ...
    print("3 - Custom")
    print("4 - Wait lemme adjust speed!")
...
def printSpeed():
    print("\n----------------")
    print("0 - 0.5 BPM (Half speed please)")
    print("1 - 1.0 BPM (Nvm, I changed my mind, normal speed please)")
    print("2 - 2.0 BPM (Double speed please)")
...
# ------------------   
import beatmapPath
input1 = beatmapPath.beatmap_path
templateCustom = []
adjustment = 1
...
printMapType()
print()
input2 = get_integer_input("What type of map to generate? [0-3]\n",0,4)
...
if input2 == 4: 
    ...
    printSpeed()
    print()
    adjustList = [0.5, 1.0, 2.0]
    adjustment = get_integer_input("What speed? [0-2]\n",0,2)
...
if input2 == 3:
    ...
    printCustomMaps()
    print()
    input3 = folderCustom[get_integer_input("Select the scaling of WE LUV LAMA. (Space aim difficulty) [0-" + str(len(folderCustom)-1) + "]\n",0,len(folderCustom)-1)]
    
    custom_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), input3)
...
# Parsing .osu
# ------------------  
filepathNewFile = input1[0:input1.index('[')]
start = 0
end = 0
bpm = 0.0
divisor = 4 * adjustment
output = []
diffName = ""
...
# Output
# ------------------
with open(filepathNewFile+"[lazyMapper "+diffName+"].osu", "w", encoding='utf-8') as file:
    for i in output:
        file.write(i)
...
print("\n\nSuccess:")
filename = beatmapPath.file[0:beatmapPath.file.index('[')]
print(filename + "[lazyMapper "+diffName+"].osu was created")
input()